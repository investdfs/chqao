...
export const insertHistoryQuestions = async () => {
  console.log("Iniciando inserção das questões de História...");
  
  // Remove questões duplicadas usando um Set com as perguntas como chave
  ...
  // Questões originais (seu array JSON aqui)
  const questions = [
    {
  ...
  try {
    // Processa e limpa as questões
    const uniqueQuestions = processQuestions(questions);
    console.log(`Processando ${uniqueQuestions.length} questões únicas...`);

    // Prepara as questões para inserção
